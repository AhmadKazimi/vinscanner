 ...
 */
internal class VinScannerRepositoryImpl(
    ...
    override suspend fun detectVinRegions(bitmap: Bitmap): List<BoundingBox> = vinDetector.detect(bitmap).boundingBoxes

    override suspend fun extractTextFromRegion(
        bitmap: Bitmap,
        boundingBox: BoundingBox,
    ): String? = textExtractor.extractText(bitmap, boundingBox)
    ...
    override fun startScanning(): Flow<VinNumber> =
        flow {
            cameraDataSource
                .startCamera()
                .map { imageProxy ->
