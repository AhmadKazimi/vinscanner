...
internal class VinDetectorImpl(
        ...
        runtime.run { interpreter ->
            ...
            try {
                ...
                if (ENABLE_DETAILED_LOGS) {
                    SLog.d(
                        TAG,
                        "Letterbox params: scaleFactor=$scaleFactor, scaled=${scaledWidth}x$scaledHeight, padding=($padLeft,$padTop)",
                    )
                }
                ...
                if (ENABLE_DETAILED_LOGS) {
                    SLog.d(
                        TAG,
                        "Output tensor shape=${outShape.contentToString()}, props=$propertiesCount, num=$numCandidates, propsFirst=$propsFirst",
                    )
                }
                ...
                fun getProp(
                    candidateIndex: Int,
                    propIndex: Int,
                ): Float = if (propsFirst) outputDynamic[0][propIndex][candidateIndex] else outputDynamic[0][candidateIndex][propIndex]
                ...
                val topIndices: java.util.PriorityQueue<Pair<Int, Float>>? =
                    if (ENABLE_DETAILED_LOGS) {
                        java.util.PriorityQueue(6) { a: Pair<Int, Float>, b -> a.second.compareTo(b.second) }
                    } else {
                        null
                    }
    ...
    private fun getOrCreateOutputBuffer(
        dimA: Int,
        dimB: Int,
