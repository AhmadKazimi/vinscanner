...
internal class CameraDataSourceImpl(
    ...
    @Synchronized
    override fun imageToBitmap(imageProxy: ImageProxy, cropRegion: BoundingBox?): Bitmap {
        val startNs = System.nanoTime()
        return when (imageProxy.format) {
            ImageFormat.YUV_420_888 -> convertYuvToBitmap(imageProxy, cropRegion).also {
                imageToBitmapTiming.log(System.nanoTime() - startNs)
            }
            ImageFormat.NV21, ImageFormat.NV16 -> convertYuvToBitmap(imageProxy, cropRegion).also {
                imageToBitmapTiming.log(System.nanoTime() - startNs)
            }
    ...
    private fun convertYuvToBitmap(imageProxy: ImageProxy, cropRegion: BoundingBox?): Bitmap {
        return try {
    ...
    private fun convertYuvToBitmapDirect(
        ...
        for (sourceRow in crop.top until crop.bottom) {
            ...
            for (sourceColumn in crop.left until crop.right) {
                val y = ((yPlaneBuffer[yRowOffset + sourceColumn].toInt() and 0xFF) - 16)
                    .coerceAtLeast(0)
    ...
    private fun rotateBitmapAndRecycle(bitmap: Bitmap, rotationDegrees: Int): Bitmap {
    ...
    private fun copyPlane(source: ByteBuffer, reusable: ByteArray): ByteArray {
...
internal fun sourceCrop(
    ...
    val normalizedSource = when (((rotationDegrees % 360) + 360) % 360) {
    ...
    val sourceLeft = kotlin.math.floor(normalizedSource[0] * imageWidth).toInt()
        .coerceIn(0, imageWidth - 1)
    val sourceTop = kotlin.math.floor(normalizedSource[1] * imageHeight).toInt()
        .coerceIn(0, imageHeight - 1)
    val sourceRight = kotlin.math.ceil(normalizedSource[2] * imageWidth).toInt()
        .coerceIn(sourceLeft + 1, imageWidth)
    val sourceBottom = kotlin.math.ceil(normalizedSource[3] * imageHeight).toInt()
