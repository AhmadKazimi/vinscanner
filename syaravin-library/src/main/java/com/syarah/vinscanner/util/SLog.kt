...
internal object SLog {
    ...
    fun v(tag: String, message: String): Int =
        if (shouldLog(tag, Log.VERBOSE)) Log.v(tag, buildRecord("TRACE", message, null)) else 0

    fun d(tag: String, message: String): Int =
        if (shouldLog(tag, Log.DEBUG)) Log.d(tag, buildRecord("DEBUG", message, null)) else 0

    fun i(tag: String, message: String): Int =
        if (shouldLog(tag, Log.INFO)) Log.i(tag, buildRecord("INFO", message, null)) else 0

    fun w(tag: String, message: String): Int =
        if (shouldLog(tag, Log.WARN)) Log.w(tag, buildRecord("WARN", message, null)) else 0

    fun w(tag: String, message: String, throwable: Throwable): Int =
        if (shouldLog(tag, Log.WARN)) Log.w(tag, buildRecord("WARN", message, throwable)) else 0

    fun e(tag: String, message: String): Int =
        if (shouldLog(tag, Log.ERROR)) Log.e(tag, buildRecord("ERROR", message, null)) else 0

    fun e(tag: String, message: String, throwable: Throwable): Int =
        if (shouldLog(tag, Log.ERROR)) Log.e(tag, buildRecord("ERROR", message, throwable)) else 0
    ...
    private fun detectCallerModule(): String {
        ...
        for (frame in frames) {
            ...
            if (
                !className.startsWith("com.syarah.vinscanner.util.SLog") &&
                className.startsWith("com.syarah.vinscanner")
            ) {
