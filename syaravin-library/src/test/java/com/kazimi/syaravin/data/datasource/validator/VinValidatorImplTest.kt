...
class VinValidatorImplTest {
    ...
    fun `validate accepts VIN with invalid checksum but valid format`() {
        // Wrong check digit (correct is 9) AND no OCR-ambiguous chars, so no permutation can
        // recover a valid checksum — exercises the soft-accept (checksumValid=false) path.
        val result = validator.validate("1HJKL7NP1R9UVW310")
